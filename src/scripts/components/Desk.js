...
const updateCardCounter = async () => {
	...
	const todo = cards.filter(card => card.column === GLOBAL_CONSTANTS.COLUMNS.TODO);
	const inProgress = cards.filter(card => card.column === GLOBAL_CONSTANTS.COLUMNS.IN_PROGRESS);
	const done = cards.filter(card => card.column === GLOBAL_CONSTANTS.COLUMNS.DONE);

	const counterTodo = document.getElementById(GLOBAL_CONSTANTS.COUNTERS.TODO);
	const counterInProgress = document.getElementById(GLOBAL_CONSTANTS.COUNTERS.IN_PROGRESS);
